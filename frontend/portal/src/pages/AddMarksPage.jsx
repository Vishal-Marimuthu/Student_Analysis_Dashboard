...
const AddMarksPage = () => {
    ...
    const showToast = (type, title, text) => {
        setToast({ type, title, text, id: Date.now() });
        setTimeout(() => setToast(prev => prev?.id === toast?.id ? null : prev), 4000);
    };

    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!selStudent || subjects.length === 0) return showToast('error', 'Incomplete', 'Select student and subjects first.');
        setLoading(true);
        ...
        try {
            ...
            if (res.ok) {
                showToast('success', 'Marks Saved', 'Student marks have been updated.');
            } else {
                showToast('error', 'Failed to Save', data.message);
            }
        } catch { showToast('error', 'Server Error', 'Unable to reach the server.'); }
